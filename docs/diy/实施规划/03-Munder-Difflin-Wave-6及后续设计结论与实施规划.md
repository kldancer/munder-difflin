...
- **皮肤**：共享同一地图拓扑、座位、碰撞、锚点和交互，只替换 Tileset、Palette、角色外观、环境覆盖层和特效语汇；适合一键切换，是 `W6.1` 的主路线。
- **独立场景**：拥有不同地图布局，例如现有 Brooklyn 99；仍可无损切换，但需要单独维护座位、寻路、咖啡区和功能锚点，优先级低于完整皮肤。

实现上不新增 Skin Engine、皮肤数据库或第二个持久字段。内置皮肤继续注册为现有 `ThemeConfig`/`ThemeId`，复用相同的 `mapRaw`、座位与锚点，仅替换视觉字段；用户选择仍保存在现有 `officeTheme`。
...
魔幻、科幻不应只是换背景色，而应把现有办公室对象翻译成同一套易懂的世界观语汇。功能位置与行为保持一致，用户无需重新学习。

| 办公室语义 | 魔法公会皮肤 | 星舰指挥舱皮肤 | 事实仍来自 |
| --- | --- | --- | --- |
| Agent 工位 | 符文研究台、漂浮羽毛笔、记忆水晶 | 舰桥控制席、全息键盘、状态舷窗 | Agent ID、座位、`status` |
| 工作显示器 | 发光水晶球或展开的魔法书 | 点亮的控制台与数据波形 | `working`、`thinking`、Tool |
| 任务板 | 冒险者委托榜、悬浮卷轴 | 全息任务沙盘、航行指令板 | `tasks.json` |
| Agent 消息 | 光球、信鸦或卷轴沿原信封轨迹飞行 | 数据脉冲、小型传信无人机 | Inbox/Outbox 路由事件 |
| 咖啡与短休 | 炼金药剂、魔法茶壶、壁炉角 | 饮料合成器、观景舷窗、休息舱 | 现有咖啡与 idle loop |
| 阻塞/等待人工 | 红色封印、求援符文 | 舱门警报、红色求援信标 | `blocked` / `blockReason` |
| Context compact | 卷轴收拢进档案箱 | 数据压缩成立方体 | `compacting` |
| 循环/断路器 | 失控法阵旋转并熄灭 | 控制台故障环与保险断开 | `looping` / Breaker |
| 完成 | 一次短促星屑/法术火花 | 一次短促全息光束/状态绿灯 | `success` |

推荐的原创皮肤组合：

| 方向 | 场景气质与创意 | 复用方式 | 建议优先级 |
| --- | --- | --- | --- |
| 纸业总部精修 | 保留当前经典办公室，统一中文状态气泡、桌面亮灭和任务板反馈 | 当前 Office 地图、Cast 与全部交互 | P0，验收基准 |
| 奥术公会 | 月光石墙面、符文工位、任务卷轴、炼金茶水间；Michael 位于公会长书房 | 复用 Office 地图、座位、咖啡与锚点，只换内置视觉资源 | P1，魔幻首选 |
| 星舰“蜂巢号”舰桥 | 深空舷窗、舰桥控制席、全息任务板、休息舱；消息像数据脉冲穿过舰桥 | 复用 Office 地图与全部状态语义，只换内置视觉资源 | P1，科幻首选；与奥术公会二选一先做 |
| 霓虹机库 | 赛博夜色、维修机械臂、无人机信使和高对比终端光；更强调 Tool 和告警状态 | 复用 Office 地图，增加少量纯 Renderer 环境覆盖层 | P2，完成首个幻想皮肤后再做 |
| 浮空炼金塔 | 云层窗景、铜管、药剂架和漂浮书页；咖啡差事自然映射为炼金日常 | 仍复用 Office 功能骨架，不新建炼金玩法 | P2，与霓虹机库择一 |
| 第九十九分局精修 | 完成现有独立地图的专属 Tileset/Cast | 复用现有 Brooklyn 99 地图合同与状态动画 | P3，独立地图维护成本更高 |

这些方向使用原创名称和原创视觉语言，不直接复刻具体影视或游戏角色。先把默认主题和“奥术公会 / 蜂巢号舰桥”中的一个做到完整，再决定第二个；不能同时铺开多个半成品。
