...
Provider 方向还必须先遵守 [Team OS 分层设计的 TOS0.5 科学协作与模型组合合同](../正式设计文档/04-Munder-Difflin个人团队操作系统与多项目工作流分层设计.md#8-科学协作与模型组合)：软件任务先确定端到端 Feature Owner 和专业能力缺口，再选最小协作拓扑及证据通道，最后选模型；能力、CLI/Hive 运行和治理三道 Gate 分开验收。候选清单不等于 Munder 当前运行能力，且不得以人物名称或厂商 benchmark 直接晋升路由或黄金主链。

| 候选方向 | 立项触发 | 增量设计结论 | 不立项信号 |
| --- | --- | --- | --- |
| Provider 策略路由 | 冻结代表性任务长期存在明显质量、遗漏、成本或时延差异 | 默认 GPT-5.6 单 Agent；按结果卡显式选择角色/模型，预览目标，失败降级可见；不静默换模型 | 差异不能稳定复现，或协调成本抵消独有证据价值 |
...
| 新 Provider | 有真实任务只有该 Provider 能明显改善，且 CLI 生命周期可验证 | 优先试点 Grok 4.6 的时效研究/开放世界挑战，再试 Kimi K3 的中文知识工作/长文档；Qwen3-Coder-Plus 条件后置；先完成能力、运行、治理三 Gate | 只有营销型号差异，缺少可靠 CLI/Hook/恢复合同，或与 Codex/DeepSeek 高度重叠 |
