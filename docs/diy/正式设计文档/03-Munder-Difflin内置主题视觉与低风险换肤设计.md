...
| 实施顺序与 Gate | 本文第 9～10 节；动态运行证据只进入 `.work/` |
...
| 前景遮挡投影 | 只包含物体正面的小型像素片 | 独立 `occlusion-geometry` Object Layer；每片显式维护对象名和 Y baseline | 脚底在物体上方时被正面像素遮挡，从下方经过时完整显示；碰撞体和地板不能冒充前景 |

构建时按 3.5 px 半径对障碍执行**障碍膨胀（Obstacle Inflation / Clearance）**后烘焙格网；运行时每一步对起点到终点执行半径一半间隔的**连续碰撞取样（Swept Collision Sampling）**，再做脚底圆与精细矩形检测，既阻止相邻合法格之间擦过桌角或薄墙，也避免渲染卡顿后的长步长跨过障碍。所有操作点必须通过入口 BFS 可达、精细碰撞净空，并用可视覆盖图核对；覆盖图中深红是原始碰撞体、浅红圆角边是 3.5 px 膨胀、紫色细格是派生导航投影、绿色圆是操作点脚底。

高保真整图不能只放在角色下方，但物理占地也不能直接作为前景。Renderer 只把 `occlusion-geometry` 中独立标注的正面窄片投影到可排序角色层，以对象 baseline 和角色脚底 `y` 值共同排序；这属于俯视 2.5D 场景常用的 **Y-sort / Foreground Occlusion**。碰撞对象与遮挡对象通过稳定对象名建立可审查关联，形状和基线各自拥有；开放地面永远不进入遮挡层。状态气泡使用固定高层级，不参与物体遮挡。
...
| `T4` | 复用 T1～T3 合同完成星穹田园公社完整地图与 15 角色包 | 第二套兼容背景/导航投影/Sprite、ThemeConfig、Picker、测试 | 不引入农场玩法、物理引擎或第二运行世界 | 4～8 天 | v2 运行合同已实现，真实运行 Gate 见 `.work` |
...
6. 在真实运行中分别让角色从物体上方和下方经过：上方经过时身体应被前景像素遮住，下方经过时应显示在物体前；同时模拟长帧，确认连续碰撞取样不会跨过薄墙。
...
- 主题选择器展示注册主题的真实地图缩略图（原版可展示其 Tileset），不得再用固定双色块冒充地图预览；缩略图只帮助识别，不改变加载和持久化流程。
...
`T0～T4` 的资产基线已经完成；星穹田园当前使用独立版本的 v2 Background 与 `.tmj`。v2 以 3.5 px 脚底圆为精细碰撞真值，在没有重复 `collision` Tile Layer 时由 Renderer 确定性投影 BFS 网格；所有岗位点和角色活动点必须同时满足脚底净空与入口可达。东侧分隔结构提供两个可见横向通道，工坊、观测台和茶歇落点不再封死入口。

高保真主题显式关闭原版办公室的悬浮任务板、问号板、归档桌与便签舞台；任务和问询仍由右侧 Command Center 承担。Michael 使用约 20 秒巡查、40 秒主管工位的简单画像，普通人员只在地图声明的员工活动点之间选目标；寻路继续只使用现有 BFS。底部 Agent 卡使用主题 `cast-atlas` 的完整正面站立帧，详情和选择器继续使用头像。中文界面下内置茶歇、errand、庆祝和状态回退使用中文池，真实命令、路径和 Agent action 原样保留。

坐姿的“身体被桌椅遮挡”和“直接裁掉角色图底部”是两个不同合同。原版办公室可继续使用旧的矩形腿部遮罩；晶海星港和星穹田园的整图背景必须关闭该遮罩，始终渲染完整角色帧，并只由独立 Foreground 像素和 Y baseline 表达遮挡。工作、坐下、起身或漫游状态切换不得改变角色素材的身体完整性。

Foreground 不再从 `collision-geometry` 自动裁出。v2 只渲染独立 `occlusion-geometry` 声明的正面窄片和 baseline；生成式透明前景若不能保持像素级配准不得接入运行时。动态命令、截图、碰撞覆盖、状态哈希和观察时长只保存在 `.work/` Gate 收据，不在正文保存一次性 pass/fail。
