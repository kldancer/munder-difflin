...
TOS0 与 TOS0.5 是运行集成前的合同切片；TOS1～TOS3 已完成有界读取、可视化投影、权威收敛和显式工作单编译。TOS4 不再作为独立实施 Wave，质量、效率、Token 和协作成本转为长期使用中的持续观察与调整。当前 Munder 会加载角色/能力结构并编译结果卡草稿，但不会自动读取项目正文、创建 Hive 任务、选择 Agent/模型或自动路由；模型目录存在仍不代表候选模型已晋升运行主链。
...
当前已完成四层权威、TOS0 文件合同、TOS0.5 科学协作/模型准入合同、TOS1 有界只读 loader、TOS2 Command Center 项目总览、TOS2.5 聚算项目归属审计，以及 TOS3 显式结果卡工作单编译。真实目录可加载 7 个组织角色和 9 个能力画像；聚算样例只携带路径与角色增量，默认授权和写集合均为空。自动路由、自动选模和 Hive 任务持久化仍未实现，它们也不再绑定到 TOS4；候选模型未晋升为新的黄金主链。后续只在真实长期使用中出现明确价值或问题时调整通用合同；命令输出与一次性收据写入 `.work/`，不得把候选能力当成已交付事实。
