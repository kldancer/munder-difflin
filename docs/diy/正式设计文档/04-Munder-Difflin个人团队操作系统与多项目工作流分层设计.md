...
TOS2.5 不搬迁 `jusuan-installer` 文件，而是在 Team OS `projects/ownership/` 记录 `retain`、`extracted`、`dedupeAfter` 三类归属。项目必须保持可独立运行，依赖方向固定为 Team OS 定位项目；通用协作语义虽已提炼，项目安全底线、机器入口和 Provider/生产约束在 TOS3/TOS4 Gate 闭合前不删除。

TOS3 在同一项目页提供显式“准备工作”入口。角色、能力和模板由 Main Process 有界校验；编译结果只存在于当前 Renderer 状态，包含业务结果、非目标、负责人角色、能力增量、项目路径引用、验收、时间预算、停止条件和授权矩阵。它不持久化第二份任务，不自动选人/模型、不直接发送、不读取项目正文，也不改变 Agent 初始 system prompt 或 Session。
...
| TOS2.5 | 固化首个项目的权威归属与去重 Gate | 禁止整体迁移；项目保持独立运行；可去重项必须等待 TOS3/TOS4 证据 |
...
TOS0 与 TOS0.5 是运行集成前的合同切片；TOS1～TOS3 已完成有界读取、可视化投影、权威收敛和显式工作单编译。当前 Munder 会加载角色/能力结构并编译结果卡草稿，但不会自动读取项目正文、创建 Hive 任务、选择 Agent/模型或自动路由；模型目录存在仍不代表候选模型已晋升运行主链。
...
当前已完成四层权威、TOS0 文件合同、TOS0.5 科学协作/模型准入合同、TOS1 有界只读 loader、TOS2 Command Center 项目总览、TOS2.5 聚算项目归属审计，以及 TOS3 显式结果卡工作单编译。真实目录可加载 7 个组织角色和 9 个能力画像；聚算样例只携带路径与角色增量，默认授权和写集合均为空。自动路由、自动选模、Hive 任务持久化与真实 workflow eval cohort 仍未实现，候选模型也未因此晋升为新的黄金主链。后续状态变化只在实现和适用验证闭合后更新本文，命令输出与一次性收据写入 `.work/`，不得把计划当成已交付能力。
