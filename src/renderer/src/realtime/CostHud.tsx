...
export function CostHud({ compact = false }: CostHudProps): React.ReactElement | null {
  ...
  if (compact) {
    ...
    return (
      <span
        title={`${totalTok.toLocaleString()} ${tr('realtime.audioTokens')} ${tr('realtime.thisSession')}`}
        style={{
          fontFamily: 'var(--cth-font-mono)',
          fontSize: 12,
          fontWeight: 600,
  ...
  return (
    <div style={wrap}>
      <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <span style={labelStyle}>{tr('realtime.spendCap')}</span>
        <input
          type="number"
          min="0"
          step="0.5"
          inputMode="decimal"
          placeholder={tr('realtime.none')}
          value={capText}
          ...
          style={capInputStyle}
        />
        <span style={{ color: 'var(--cth-ink-500)' }}>USD{capUsd != null ? '' : ` (${tr('realtime.off')})`}</span>
      </label>

      {live ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <span style={{ color: meterColor, fontWeight: 600 }}>
            {formatUsd(usd)} {tr('realtime.thisSession')}{capUsd != null ? ` / ${formatUsd(capUsd)}` : ''}
          </span>
          <span style={{ color: 'var(--cth-ink-500)', fontSize: 11 }}>
            {inputTokens.toLocaleString()} in · {outputTokens.toLocaleString()} out {tr('realtime.audioTokens')}
          </span>
          {overCap && (
          ...
          {near && (
            <span style={{ color: 'var(--cth-warn, #b8860b)', fontSize: 11 }}>
              {tr('realtime.approachingCap')}
            </span>
          )}
        </div>
      ) : (
        <span style={{ color: 'var(--cth-ink-500)', fontSize: 11 }}>
          {usd > 0 ? tr('realtime.lastSession', { amount: formatUsd(usd) }) : tr('realtime.noActiveSession')}
        </span>
      )}
    </div>
  );
