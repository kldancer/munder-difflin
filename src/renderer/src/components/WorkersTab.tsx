...
function StatusBadge({ w }: { w: WorkerSnapshot }) {
  ...
  return (
    <span style={{
      ...
      {releasing ? t('workers.stoppingStatus') : t('workers.working')}
    </span>
  );
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '12px 14px 16px', overflow: 'auto' }}>
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          <span style={sectionHead}>{t('workers.live')}</span>
              ...
              <div key={w.workerId} style={card}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                    ...
                    {w.hasSlack && (
                      <span title={t('workers.slackTitle')} style={{
                  ...
                  >
                    {w.releasing || stopping[w.workerId] ? t('workers.stopping') : t('workers.stop')}
                  </PixelButton>
                </div>
                <div style={metaRow}>
                  <span title={t('workers.workerId')}>{w.workerId}</span>
                  <span>{t('workers.base', { branch: w.baseBranch })}</span>
                  <span>{t('workers.up', { age: relAge(w.ageMs) })}</span>
                  <span>
                    {w.idleMs === null ? t('workers.ptyGone') : t('workers.idle', { age: relAge(w.idleMs) })}
                  </span>
                  <span>
                    {t('workers.tokens', { used: fmtTokens(w.tokensUsed) })}{w.tokenCap !== null ? ` / ${fmtTokens(w.tokenCap)}` : t('workers.uncapped')}
                  </span>
                </div>
              </div>
            ))}
      ...
      {preserved.length > 0 && (
        <div>
          <span style={sectionHead}>{t('workers.preserved', { count: preserved.length })}</span>
          <p style={{ fontFamily: 'var(--cth-font-ui)', fontSize: 11, color: 'var(--cth-ink-700)', margin: '2px 0 8px' }}>
            {t('workers.preservedDescription')}
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {preserved.map((p) => (
              <div key={p.wtPath} style={card}>
                ...
                <div style={metaRow}>
                  ...
                  <span>{t('workers.base', { branch: p.baseBranch })}</span>
                  <span>{t('workers.kept', { age: relAge(Math.max(0, Date.now() - p.preservedAt)) })}</span>
                </div>
              </div>
            ))}
          </div>
