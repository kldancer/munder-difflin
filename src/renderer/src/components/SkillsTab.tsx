...
export function SkillsTab({ agentCwd }: { agentCwd?: string }) {
  ...
  const loadCatalog = useCallback(async (force = false) => {
    ...
    try {
      ...
      setCatalog([]);
      setCatalogMeta({ stale: true, error: 'catalog-unreachable', fetchedAt: 0 });
  ...
  const install = async (c: CatalogSkill) => {
    ...
    try {
      ...
      if (res.ok) {
        setAction((a) => ({ ...a, [c.url]: { done: t('skills.installedDone') } }));
        void loadLocal(); // the installed pane must reflect it immediately
      } else {
        setAction((a) => ({ ...a, [c.url]: { error: res.error } }));
      }
    } catch (e) {
      setAction((a) => ({ ...a, [c.url]: { error: e instanceof Error ? e.message : t('skills.failed') } }));
    }
  };
  ...
  const uninstall = async (sk: LocalSkill) => {
    ...
    try {
      ...
      else setAction((a) => ({ ...a, [sk.path]: { error: res.error ?? t('skills.removeFailed') } }));
    } catch (e) {
      setAction((a) => ({ ...a, [sk.path]: { error: e instanceof Error ? e.message : t('skills.removeFailed') } }));
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, height: '100%' }}>
      ...
      <div style={{
        ...
        <button onClick={() => setMode('installed')} style={tabBtn('installed', t('skills.installed'))}>
          {t('skills.installed')}{local ? ` (${local.length})` : ''}
        </button>
        <button onClick={() => setMode('browse')} style={tabBtn('browse', t('skills.browse'))}>
          {t('skills.browse')}{catalog ? ` (${catalog.length})` : ''}
        </button>
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder={mode === 'installed' ? t('skills.installedSearch') : t('skills.catalogSearch')}
          style={{
        ...
        {mode === 'browse' && (
          ...
          >
            <option value="all">{t('skills.categories')}</option>
        ...
        {mode === 'browse' && (
          ...
          >
            <option value="all">{t('skills.publishers')}</option>
        ...
        <PixelButton
          ...
          disabled={busy}
        >{busy ? t('skills.loading') : t('skills.refresh')}</PixelButton>
      </div>

      {/* Body */}
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 10 }}>
        {mode === 'installed' ? (
          local === null ? <Muted>{t('skills.loading')}</Muted>
          : shownLocal.length === 0 ? (
            <Muted>
              {local.length === 0 ? t('skills.empty') : t('skills.noMatch')}
            </Muted>
          ) : (
                    ...
                    {s.scope === 'bundled' ? (
                      ...
                      <>
                        <button onClick={() => void uninstall(s)} style={actionBtn('danger')}>
                          {t('skills.uninstall')} {s.name}?
                        </button>
                        <button onClick={() => setConfirming(null)} style={actionBtn('quiet')}>{t('skills.cancel')}</button>
                      </>
                    ) : (
                      <button
                      ...
                      >{action[s.path]?.busy ? t('skills.uninstalling') : t('skills.uninstall')}</button>
                    )}
                    {action[s.path]?.error && (
        ...
        ) : catalog === null ? <Muted>{t('skills.loading')}</Muted>
        : (
          <>
            {catalogMeta?.error && (
              <div style={{
                ...
                {t('skills.stale')} {catalogMeta.error === 'catalog-unreachable' ? t('skills.catalogFailed') : catalogMeta.error}.
              </div>
            )}
            <div style={{ fontSize: 11, color: 'var(--cth-ink-500)', marginBottom: 8 }}>
              ...
              {' · '}{t('skills.curated')} abubakarsiddik31/claude-skills-collection
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {shownCatalog.map((s) => (
