...
const MODELS: { id: ModelId; titleKey: string; detailKey: string }[] = [
  { id: 'minilm', titleKey: 'memory.fast', detailKey: 'memory.englishOnly' },
  { id: 'embeddinggemma', titleKey: 'memory.multilingual', detailKey: 'memory.allLanguages' },
];
...
export function MemoryPanel() {
  ...
  const run = async () => {
    ...
    try {
      ...
      setResult(res.ok ? (res.output || t('memory.nothing')) : t('memory.searchFailed', { error: res.error }));
    } finally {
      setBusy(false);
    }
  };
  ...
  const pill = active ? `${t('memory.pill')} · ${status?.model}` : t('memory.pill');

  // One clear state line: is memory working, off, or not set up?
  const state: { dot: string; label: string } = !status?.available
    ? { dot: 'var(--cth-coral)', label: t('memory.notSetUp') }
    : !status.enabled
      ? { dot: 'var(--cth-ink-500)', label: t('memory.off') }
      : status.initialized
        ? { dot: 'var(--cth-mint)', label: t('memory.ready') }
        : { dot: 'var(--cth-lemon)', label: t('memory.preparing') };
  ...
  return (
    <div style={{ position: 'absolute', bottom: 12, left: 12, width: open ? 380 : 'auto', zIndex: 40 }}>
      {!open ? (
        ...
        <PixelPanel variant="dialog" title={t('memory.title')} noPadding>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
              ...
              {status?.available && (
                ...
                >
                  {status.enabled ? t('memory.turnOff') : t('memory.turnOn')}
                </PixelButton>
              )}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid var(--cth-ink-300)', paddingTop: 10 }}>
              <PixelButton variant="ghost" size="sm" onClick={() => setOpen(false)}>{t('memory.close')}</PixelButton>
            </div>
          </div>
        </PixelPanel>
      )}
