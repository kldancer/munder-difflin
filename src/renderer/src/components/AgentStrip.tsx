    ...
    <div style={{
              ...
              <>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    }}>{t('extra.privateNote', { name: a.name.toUpperCase() })}</span>
                    <button
                      onClick={() => setNoteEditId(null)}
                      title={t('agent.done')}
                      aria-label={t('agent.closeNote')}
                      style={{
                  ...
                  <textarea
                    ...
                    aria-label={t('agent.noteFor', { name: a.name })}
                    style={{
      ...
      >
        <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center', whiteSpace: 'nowrap' }}>
          <Icon name="plus" /> {t('extra.addAgent')}
        </span>
      </PixelButton>
      ...
      {(restorableAgents.length > 0 || restoreBusy) && (
        <span
          ...
          title={restoreBusy
            ? t('extra.restoringTip')
            : t('extra.previousSession', { names: restorableAgents.map((a: Agent) => a.name).join(', ') })}
        >
          <PixelButton
            variant="primary"
            size="lg"
          ...
          >
            <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center', whiteSpace: 'nowrap' }}>
              <Icon name="play" />
              {restoreBusy ? t('extra.restoring') : t('extra.restoreTeam', { count: restorableAgents.length })}
            </span>
          </PixelButton>
        </span>
      )}
        ...
        <>
          ...
          <div style={{
            ...
            {restorableAgents.map((a: Agent) => (
              <span
                key={a.id}
                title={t('agent.restorable', { name: a.name })}
                style={{
              ...
              >
                ...
                <button
                  ...
                  title={t('agent.dismiss', { name: a.name })}
                  aria-label={t('agent.dismiss', { name: a.name })}
                  style={{
                    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
