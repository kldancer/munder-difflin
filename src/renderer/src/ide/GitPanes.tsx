  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
        ...
        {!loading && commits.length === 0 && <div style={noteStyle}>{t('gitPanes.noCommits')}</div>}
      ...
      {selected && (
        <div style={{
          ...
          <div style={{
            ...
            <button style={{ ...smallBtn, width: 20, justifyContent: 'center' }} onClick={() => setSelected(null)} title={t('gitPanes.close')}>✕</button>
          </div>
...
export function ComparePane({ gitRoot, onOpenRevDiff }: {
  ...
  const switchTo = async () => {
    if (!head) return;
    if (!window.confirm(t('gitPanes.switchConfirm', { branch: head }))) return;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '6px 12px', flexShrink: 0 }}>
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          ...
          <button style={{ ...smallBtn, width: 22, justifyContent: 'center' }} title={t('gitPanes.swap')}
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 11, color: 'var(--cth-ink-500)' }}>
          ...
          <button
            ...
            title={mode === 'three'
              ? t('gitPanes.prModeTitle')
              : t('gitPanes.twoModeTitle')}
          >{mode === 'three' ? t('gitPanes.sinceAncestor') : t('gitPanes.literalDifference')}</button>
          <span style={{ flex: 1 }} />
          <button style={smallBtn} onClick={() => void switchTo()} title={t('gitPanes.checkoutBranch', { branch: head })}>
