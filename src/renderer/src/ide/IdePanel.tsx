    ...
    <div style={{
      ...
      >
        ...
        {target.agent ? (
          <span style={{ display: 'inline-flex', alignItems: 'baseline', gap: 6, minWidth: 0 }}>
            <span
              ...
              ? t('ideApp.inferredTitle', { name: target.agent.name })
                : t('ideApp.workspaceTitle', { name: target.agent.name })}
              style={{
        ...
        <span title={root ?? ''} style={{
          ...
          {root ? basename(root) : t('app.noWorkspace')}
        </span>
        <button
          className="cth-titlebar-nodrag"
          onClick={() => setIdeOpen(false)}
          title={t('app.closeIde')}
          aria-label={t('app.closeIdeLabel')}
          style={{
            marginLeft: 'auto',
      ...
      {!root ? (
        <div style={{
          ...
          {t('app.noWorkspaceMessage')}<br />{t('app.noWorkspaceDetail')}
        </div>
      ) : (
          ...
          <div style={{
            ...
            <div style={{
              ...
              <button
                ...
                title={gitCollapsed ? t('ideApp.expandGit') : t('ideApp.collapseGit')}
                aria-label={gitCollapsed ? t('ideApp.expandGit') : t('ideApp.collapseGit')}
              ...
              {railTab === 'changes' && !gitCollapsed && (
                <button onClick={() => refreshStatus()} title={t('ideApp.refresh')} style={iconBtn}>
                  <Icon name="web" />
                </button>
              )}
            </div>
            ...
            <div style={{ flexShrink: 0, maxHeight: '45%', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
              ...
              <div style={{ flex: 1, overflow: 'auto', minHeight: 0 }}>
                {isRepo === false && (
                  <div style={{ padding: '6px 12px', fontSize: 12, color: 'var(--cth-ink-500)' }}>{t('ideApp.notRepo')}</div>
                )}
                {isRepo && changedFiles.length === 0 && (
                  <div style={{ padding: '6px 12px', fontSize: 12, color: 'var(--cth-ink-500)' }}>{t('ideApp.clean')}</div>
                )}
                {changedFiles.map((f) => {
            ...
            <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', borderTop: '1px solid var(--cth-ink-300)' }}>
              <SectionHeader title={t('ideApp.files')} />
          ...
          <div style={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column', background: 'var(--cth-paper-100)' }}>
                    ...
                    {tab.mode !== 'edit' && (
                      <span style={{
                        ...
                        background: tab.mode === 'revdiff' ? 'var(--cth-lilac-light)'
                          ...
                          : 'var(--cth-sky-light)',
                        color: 'var(--cth-ink-900)'
                      }}>{tab.mode === 'revdiff' ? (tab.revLabel ?? 'REV') : tab.mode === 'image' ? 'IMG' : 'DIFF'}</span>
                    )}
            ...
            <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
              ...
              {activeTab?.mode === 'edit' && (() => {
                ...
                if (!buf || buf.status === 'loading') return <Centered>{t('ideApp.loading')}</Centered>;
              ...
              {activeTab?.mode === 'revdiff' && (() => {
                ...
                if (!d || d.status === 'loading') return <Centered>{t('ideApp.loadingDiff')}</Centered>;
                if (d.status === 'error') return <Centered tone="error">{d.error}</Centered>;
                if (d.status === 'binary') return <Centered>{t('ideApp.binary')}</Centered>;
                return (
                  <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                    <div style={{
                ...
                return (
                  <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                    <div style={{
                      ...
                      <span style={{ color: 'var(--cth-ink-500)' }}>{t('ideApp.head')}</span>
                      <Icon name="arrow-right" />
                      <span>{t('ideApp.workingTree')}</span>
                      <span style={{
                        flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                        fontFamily: 'var(--cth-font-mono)', textAlign: 'right'
                      }} title={activeTab.rel}>{activeTab.rel}</span>
                      <button onClick={() => ensureDiff(activeTab.rel, true)} title={t('ideApp.refreshDiff')} style={iconBtn}>
                        <Icon name="web" />
                      </button>
                    </div>
                    <div style={{ flex: 1, minHeight: 0 }}>
...
function EditorBar({ rel, dirty, saveState, onSave, onCopy, mdView, onMdView, onViewImage }: {
  ...
  return (
    <div style={ideBarStyle}>
        ...
        <span style={{ display: 'inline-flex', gap: 0 }}>
          {(['code', 'split', 'preview'] as const).map((v) => (
            <button
              key={v}
              onClick={() => onMdView(v)}
              title={v === 'code' ? t('ideApp.editor') : v === 'split' ? t('ideApp.split') : t('ideApp.preview')}
              style={{
                ...textBtn,
      ...
      {onViewImage && (
        <button onClick={onViewImage} title={t('ideApp.viewImage')} style={textBtn}>{t('ideApp.viewImage')}</button>
      )}
      <button onClick={onCopy} title={t('ide.copyAbsolutePath')} style={textBtn}>{t('ide.copyPath')}</button>
      <button onClick={onSave} disabled={!dirty || saveState === 'saving'} title={t('ideApp.save')}
